...
#### 7. Device App Analysis (3 tools)

> Inspect installed apps and export APKs for static analysis by AI on a computer. No root required.

| Tool | Description |
|------|-------------|
| `list_installed_apps` | List installed apps (package name, label, version, system app flag, install time, APK size) |
| `get_app_metadata` | Get app metadata: permissions (with protection level), Activity/Service/Receiver/Provider components, signature certificate fingerprints (MD5/SHA1/SHA256), SDK versions |
| `export_apk` | Export an app's APK and return an HTTP download URL; download it on your computer and analyze with jadx/apktool (supports split APKs as zip) |

**Typical workflow:**
```
AI  → list_installed_apps keyword="app name"    ← find the target package name
AI  → get_app_metadata packageName="com.xxx"    ← analyze permissions, components, signature
AI  → export_apk packageName="com.xxx"          ← get the download URL
PC  → curl -O http://192.168.1.5:9010/apk/<token> ← download the APK for jadx/apktool analysis
```
