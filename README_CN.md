...
> 获取设备上安装应用的信息、导出 APK 供电脑端 AI 静态分析，无需 root。

| 工具 | 说明 |
|------|------|
| `list_installed_apps` | 列出已安装应用（包名、应用名、版本、是否系统应用、安装时间、APK 大小） |
| `get_app_metadata` | 获取应用元数据：权限（含保护级别）、Activity/Service/Receiver/Provider 组件、签名证书指纹（MD5/SHA1/SHA256）、SDK 版本等 |
| `export_apk` | 导出应用 APK 安装包，返回 HTTP 下载 URL，电脑端下载后可用 jadx/apktool 分析（支持 split APK 打包） |

**典型工作流：**
```
AI → list_installed_apps keyword="目标应用名"     ← 找到目标应用包名
AI → get_app_metadata packageName="com.xxx.app"   ← 分析权限、组件、签名
AI → export_apk packageName="com.xxx.app"         ← 获取下载链接
电脑 → curl -O http://192.168.1.5:9010/apk/<token> ← 下载 APK 用 jadx/apktool 分析
```
