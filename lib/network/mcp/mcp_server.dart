...
class McpServer {
  ...
  /// 已导出的 APK 文件（token -> 导出条目，供 /apk/<token> 下载）
  final Map<String, _ApkExportEntry> _apkExports = {};

  /// APK 导出下载链接有效期
  static const Duration apkExportTtl = Duration(minutes: 30);
  ...
  // ─────────────────────────────────────────────────────────────
  // APK 导出与下载（供 AI 客户端下载 APK 到电脑分析）
  // ─────────────────────────────────────────────────────────────

  /// 注册一个已导出的 APK 文件，返回下载路径（/apk/<token>）
  String registerApkExport(String filePath, String fileName) {
    _cleanExpiredApkExports();
    final token = _generateSessionId();
    _apkExports[token] = _ApkExportEntry(token, filePath, fileName, DateTime.now());
    logger.i('MCP APK export registered: $fileName (${_apkExports.length} active)');
    return '/apk/$token';
  }

  /// 获取设备本机局域网 IPv4 地址列表（用于拼接下载 URL）
  Future<List<String>> getLocalIps() async {
    try {
      ...
      return interfaces
          ...
          .where((addr) => !addr.startsWith('127.'))
          .toList();
    } catch (_) {
      return [];
    }
  ...
  /// 清理过期导出条目并删除对应文件
  void _cleanExpiredApkExports() {
    final now = DateTime.now();
    final expired = _apkExports.entries
        .where((e) => now.difference(e.value.createdAt) > apkExportTtl)
        .map((e) => e.key)
        .toList();
    for (var token in expired) {
      final entry = _apkExports.remove(token);
      if (entry != null) {
        try {
          io.File(entry.filePath).delete();
        } catch (_) {}
      }
    }
  }

  /// 处理 GET /apk/<token>：流式返回导出的 APK 文件
  Future<void> _handleApkDownload(io.HttpRequest request, String token) async {
    _cleanExpiredApkExports();
    final entry = _apkExports[token.trim()];
    ...
    if (entry == null) {
      await notFound('APK export not found or expired (TTL ${apkExportTtl.inMinutes} minutes). '
          'Please call the export_apk MCP tool again to get a fresh download URL.');
      return;
    }

    final file = io.File(entry.filePath);
    if (!await file.exists()) {
      _apkExports.remove(token);
      await notFound('APK file missing on device. Please call the export_apk MCP tool again.');
      return;
    }

    try {
      final length = await file.length();
      final contentType = entry.fileName.endsWith('.zip')
          ? io.ContentType('application', 'zip')
          : io.ContentType('application', 'vnd.android.package-archive');
      request.response.statusCode = io.HttpStatus.ok;
      request.response.headers.contentType = contentType;
      request.response.headers.set('Content-Disposition', 'attachment; filename="${entry.fileName}"');
      request.response.contentLength = length;
      await file.openRead().pipe(request.response);
      logger.i('MCP APK downloaded: ${entry.fileName} ($length bytes)');
    } catch (e) {
      logger.e('MCP APK download error: $e');
      try {
  ...
  Future<void> start() async {
    ...
    while (attempts < maxAttempts) {
      try {
        // 绑定 0.0.0.0 以允许局域网访问（需要在 AndroidManifest 中允许 cleartext）
        final bindAddr = io.InternetAddress.anyIPv4;
  ...
  Future<void> stop() async {
    ...
    // 清理已导出的 APK 文件
    for (var entry in _apkExports.values) {
      try {
        io.File(entry.filePath).delete();
      } catch (_) {}
    }
  ...
  void _handleRequest(io.HttpRequest request) {
    ...
    // APK 文件下载（供 AI 客户端从电脑下载导出的 APK）
    if (path.startsWith('/apk/') && method == 'GET') {
      _handleApkDownload(request, path.substring('/apk/'.length));
      return;
    }
...
/// 已导出的 APK 下载条目
class _ApkExportEntry {
  final String token;
  final String filePath;
  final String fileName;
  final DateTime createdAt;

  _ApkExportEntry(this.token, this.filePath, this.fileName, this.createdAt);
}
