...
import 'package:proxypin/native/app_analysis.dart';
...
class McpTools {
  ...
  static List<Map<String, dynamic>> getToolDefinitions() {
    return [
      ...
      // ── F. 设备应用分析（元数据 + APK 导出）───────────────────────────────
      {
        ...
        'description':
            '列出 Android 设备上已安装的应用（包名、应用名、版本、是否系统应用、安装/更新时间、APK 大小）。用于查找目标应用的包名，配合 get_app_metadata 和 export_apk 使用。无需 root。',
        'inputSchema': {
          'type': 'object',
          'properties': {
            'includeSystemApps': {'type': 'boolean', 'description': '是否包含系统应用，默认 false'},
            'keyword': {'type': 'string', 'description': '按包名或应用名模糊过滤，例如 "wechat"'},
          },
        },
      },
      {
        ...
        'description':
            '获取指定应用的详细元数据：版本信息、minSdk/targetSdk、权限列表（含保护级别 normal/dangerous/signature）、四大组件（Activity/Service/Receiver/Provider 及 exported 状态）、签名证书（MD5/SHA1/SHA256 指纹、签发者、有效期）、APK 路径与大小、manifest meta-data 等。适用于应用静态信息分析，无需 root。',
        'inputSchema': {
          'type': 'object',
          'properties': {
            'packageName': {'type': 'string', 'description': '应用包名，从 list_installed_apps 获取'},
          },
      ...
      },
      {
        'name': 'export_apk',
        'description':
            '导出指定应用的 APK 安装包文件，返回 HTTP 下载 URL，可在电脑上下载后用 jadx/apktool 等工具进行静态分析。默认导出 base.apk；应用为 split APK 且 includeSplits=true 时导出包含全部分片的 zip 包。下载链接 30 分钟内有效，无需 root。',
        'inputSchema': {
          'type': 'object',
          'properties': {
            'packageName': {'type': 'string', 'description': '应用包名'},
            'includeSplits': {'type': 'boolean', 'description': '是否导出包含所有 split APK 的 zip 包，默认 false'},
          },
          'required': ['packageName'],
        },
      },
  ...
  static Future<Map<String, dynamic>> callTool(
    ...
    // F. 设备应用分析工具不依赖抓包数据容器，先于空检查分发
    switch (toolName) {
      case 'list_installed_apps':
        return _listInstalledApps(arguments);
      case 'get_app_metadata':
        return _getAppMetadata(arguments);
      case 'export_apk':
        return _exportApk(arguments);
    }
  ...
  static Future<Map<String, dynamic>> _listInstalledApps(Map<String, dynamic> args) async {
    final includeSystemApps = (args['includeSystemApps'] as bool?) ?? false;
    final keyword = (args['keyword'] as String?) ?? '';

    try {
      final apps = await AppAnalysis.listApps(includeSystemApps: includeSystemApps, keyword: keyword);
      return _toolResult({
        'total': apps.length,
        'includeSystemApps': includeSystemApps,
        'apps': apps,
        'hint': '使用 get_app_metadata 查看应用详情，或用 export_apk 导出 APK 安装包到电脑分析',
      });
    } catch (e) {
      return _toolError('获取应用列表失败: $e');
    }
  }

  static Future<Map<String, dynamic>> _getAppMetadata(Map<String, dynamic> args) async {
    final packageName = args['packageName'] as String?;
    if (packageName == null || packageName.isEmpty) return _toolError('packageName 参数不能为空');

    try {
      final metadata = await AppAnalysis.getAppMetadata(packageName);
      return _toolResult(metadata);
    } catch (e) {
      return _toolError('获取应用元数据失败: $e');
    }
  }

  static Future<Map<String, dynamic>> _exportApk(Map<String, dynamic> args) async {
    final packageName = args['packageName'] as String?;
    if (packageName == null || packageName.isEmpty) return _toolError('packageName 参数不能为空');
    final includeSplits = (args['includeSplits'] as bool?) ?? false;

    try {
      final info = await AppAnalysis.exportApk(packageName, includeSplits: includeSplits);
      final server = McpServer.instance;
      final downloadPath = server.registerApkExport(info['path'] as String, info['fileName'] as String);
      final ips = await server.getLocalIps();
      ...
      return _toolResult({
        ...
        'includeSplits': info['includeSplits'],
        'splitCount': info['splitCount'],
        'downloadPath': downloadPath,
        'serverPort': server.port,
        'serverIps': ips,
        'downloadUrls': ips.map((ip) => 'http://$ip:${server.port}$downloadPath').toList(),
        'expiresInMinutes': 30,
        'hint': '在电脑上通过任一 downloadUrl 下载（如 curl -O <url>），下载后可用 jadx/apktool 等工具静态分析',
      });
    } catch (e) {
      return _toolError('导出 APK 失败: $e');
    }
  }
