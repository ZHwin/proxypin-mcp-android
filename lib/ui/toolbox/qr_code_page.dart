...
class _QrDecodeState extends State<_QrDecode> with AutomaticKeepAliveClientMixin {
  ...
  Future<String?> selectImage() async {
    ...
    if (Platforms.isDesktop()) {
      //<String>['jpg', 'png', 'jpeg']
      List<PlatformFile> files = await FilePicker.pickFiles(type: FileType.image);
...
class _QrEncodeState extends State<_QrEncode> with AutomaticKeepAliveClientMixin {
  ...
  Future<void> saveImage() async {
    ...
    Uri? uri = await FilePicker.saveFile(fileName: "qrcode.png", bytes: imageBytes, type: FileType.image);
    if (uri == null) return;
    if (mounted) {
